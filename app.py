...
# قائمة المكتبات التي يحتاجها السورس (بالأسماء المستخدمة في الاستيراد)
required_packages = [
    'threading', 'jwt', 'random', 'json', 'requests', 'google.protobuf',
    'protobuf_decoder', 'datetime' , 'base64',
    'logging', 're', 'socket', 'binascii', 'psutil', 'Crypto', 'urllib3', 'flask', 'flask_cors', 'uuid'
]

# أسماء الحزم على PyPI (تختلف أحياناً عن اسم الاستيراد)
pip_packages = {
    'jwt': 'pyjwt',
...
def install_and_import(package):
    try:
        ...
        print(f"⚠️  {package} غير موجود، جاري التثبيت...")
        pip_name = pip_packages.get(package, package)
        subprocess.check_call([sys.executable, "-m", "pip", "install", pip_name])
        print(f"✅ تم تثبيت {package}")

# تثبيت كل المكتبات المطلوبة قبل بدء الكود الأصلي
